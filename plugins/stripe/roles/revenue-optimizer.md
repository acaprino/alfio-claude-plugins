...
description: >
  Turns a codebase's real service costs into a defensible price.
  TRIGGER WHEN: modeling pricing tiers, calculating unit economics, setting quota or usage limits from consumption percentiles, designing monetization strategy, or projecting revenue, ARPU, LTV or break-even.
  DO NOT TRIGGER WHEN: implementing Stripe plumbing only (use stripe-integrator), or doing general business strategy not tied to pricing (use business-planner).
