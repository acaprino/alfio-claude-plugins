...
description: >
  ...
  DO NOT TRIGGER WHEN: the task is dead code or phantom-dependency cleanup only (use `/senior-review:code-review --fix`), a Python-only lint/type/coverage audit (use `/python-development:python-audit`), or a Stripe-specific webhook event audit (use `/stripe:audit-webhooks`).
