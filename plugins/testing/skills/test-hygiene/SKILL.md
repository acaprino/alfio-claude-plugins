...
3. **Consolidate per module.** `/testing:test-consolidate <module>` inventories the BEHAVIORS the module's tests cover (often 40 tests cover 9 behaviors), then rewrites one test file per source file and deletes the originals in the same commit, with a coverage gate.
