...
| 2. Adversarial review | Spawns one teammate per dimension in parallel, each reading the X-ray run directory, the interconnect map and the knowledge provenance, and declaring the load-bearing premise and provenance of every finding |
| 3. Consolidation | Delivery gate first (every reviewer delivered or is reported degraded), then deduplicates findings, resolves severity conflicts, weighs agreement by premise provenance (independent agreement corroborates, shared-premise agreement is an echo), collects `[MAP-GAP]` findings as mapper coverage gaps, organizes by severity |
