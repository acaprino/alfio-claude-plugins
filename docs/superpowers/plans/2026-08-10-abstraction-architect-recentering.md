...
| 13 | `13-ownership-is-per-decision.md` | Canonical ownership can be assigned per decision, not only per file. Stimulus: two authoritative specifications describe the same CLI command. One defines the output format correctly, matching the surrounding codebase's convention, but omits a required schema field. The other defines the required field correctly but also specifies an incompatible output format. PASS: classified **D2**, with a remediation that assigns ownership per decision (output format to the first, required fields to the second). FAIL: any of these three: classifying it as D1 or D3, since duplication is the symptom and contested ownership is the defect; declaring one document authoritative in full, which discards a fact the other one owns correctly; or resolving by a source-ranking heuristic such as "prefer the more recent, the more specific, or the one closer to the code", which is the degeneration this case exists to prevent. |
