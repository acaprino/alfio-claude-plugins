...
2. Pick the most direct path: search for discovery, `WebFetch` for extraction; with `SERPER_API_KEY` set, `python ${CLAUDE_PLUGIN_ROOT}/skills/web-search-techniques/scripts/websearch.py "<query>"` is an equivalent discovery step
...
- **Bash**: only for `${CLAUDE_PLUGIN_ROOT}/skills/web-search-techniques/scripts/websearch.py` (serper backend) and `${CLAUDE_PLUGIN_ROOT}/skills/web-search-techniques/scripts/webfetch.py` (bot-block fallback).
