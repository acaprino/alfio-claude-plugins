...
> Arguments: `<plugin-name> [--with-agent] [--with-skill] [--with-command] [--category <cat>] [--author <name>]`. Wherever `<arguments>` appears below, substitute the text the user typed after the skill name.
