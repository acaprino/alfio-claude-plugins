...
> Arguments: `<prompt text or file path> [--model claude|gpt|gemini] [--optimize-for clarity|tokens|reliability] [--compare]`. Wherever `<arguments>` appears below, substitute the text the user typed after the skill name.
...
Agent:
  ...
  prompt: |
    ...
    defaults" section of `<plugin-root>/references/reasoning-patterns.md` before adding
    ...
    If yes, read `<plugin-root>/references/reasoning-patterns.md`, pick the pattern
