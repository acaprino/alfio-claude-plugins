...
5. **Resolve the challenger profile.** Call the `mcp__peer-review__peer_profiles` tool
   ...
   - If `default` is null and no `--challenger` was given: stop. No profile is
     ...
     found at all). Point at `<plugin-root>/mcp/profiles.example.json`, the
   ...
   - If the chosen name does not appear in `profiles`: stop, listing the configured
     ...
     actually loaded), and pointing at `<plugin-root>/mcp/profiles.example.json`
   ...
   - If the chosen profile's `available` is `false`: stop, naming its `api_key_env`
     ...
     `<plugin-root>/mcp/profiles.example.json`.
...
1. Load the Round 1 prompt: read `<plugin-root>/protocol/round-prompts.md` and
...
6. **Initialize `03-ledger.md`.** Parse the `## Findings` section of
   ...
   from `<plugin-root>/protocol/finding-lifecycle.md`:
...
1. Load the Challenge round prompt: read
   `<plugin-root>/protocol/round-prompts.md`, extract the fenced block under
   `## Challenge round (2..N)`.
...
2. Load the Certification prompt: read `<plugin-root>/protocol/round-prompts.md`
