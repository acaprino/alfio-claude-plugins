...
> Arguments: `<target path> [--critical] [--comments] [--docs-only] [--phase N] [--depth=lite|full] [--run-name <name>]`. Wherever `<arguments>` appears below, substitute the text the user typed after the skill name.
