...
Read `.github/skills/abstraction-architect/SKILL.md`. Read `references/dimensions.md` first; read the rest on demand.
...
Mode `diff` needs only `01-structure.md` and `02-interfaces.md`, both produced by `--depth=lite`. With no deep-dive output at all, `diff` runs on the concept index plus `#search/fileSearch` and `#search/textSearch`, at reduced confidence, and says so in Gaps.

# PROCESS (mode = global)

1. **Load the skill.** Read `.github/skills/abstraction-architect/SKILL.md`, then `references/dimensions.md`.
...
3. **Load the concept index and check freshness.** `$SKILLS` is the installed skills directory: the first of `.github/skills/`, `.agents/skills/`, `.claude/skills/`, `~/.copilot/skills/` that exists. Write `changed_files` to a temporary file, one path per line, then run the script with that file's path as `--changed-files`:

   ```bash
   python "$SKILLS/abstraction-architect/scripts/concept_index.py" \
...
- **Dedup with `review-code-auditor`**, which runs as the Architecture dimension of the same review and owns smells visible inside one file. Yours is the cross-file question. See `references/scope-boundaries.md`.
