...
- Search `open\(|createReadStream|createWriteStream|socket|connect\(|getConnection|acquire|new Worker|spawn|subprocess|Popen|exec\(` -- handles, connections, processes
- Search `addEventListener|\.on\(|subscribe|watch\(|observe` -- listeners and subscriptions
- Search `lock|mutex|semaphore|RwLock|Lock\(\)` -- synchronization resources
- Search `setInterval|setTimeout|requestAnimationFrame|createObjectURL|mmap|malloc|Box::leak` -- timers, URLs, raw memory
- Search `go func|tokio::spawn|asyncio\.create_task|Promise\.|Thread\(|ensure_future` -- concurrent task creation
