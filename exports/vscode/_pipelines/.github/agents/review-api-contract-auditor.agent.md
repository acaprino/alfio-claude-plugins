...
- **The interconnect map** (`.team-review/02-interconnect.md`) when produced by `xray-interconnect-mapper` -- specifically the `## Contracts` (formal) section
...
- Manual types: search for request/response type definitions that reference API shapes; check field-by-field
...
- Source-of-truth map for what contracts exist -> `xray-interconnect-mapper` output at `.team-review/02-interconnect.md` (`## Contracts` section)
