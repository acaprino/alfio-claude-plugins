...
`$SKILLS/abstraction-architect/scripts/concept_index.py`
...
Review delta sources, in precedence order: `--changed-files` (one path per line, which is how `/team-review` passes scope), then `--base` with optional `--head`, then `--working-tree`. With none of them the review delta is empty and only the baseline drift drives revalidation.
...
Plain text rather than JSON is deliberate: `validate` has no machine consumer (the agent invokes `status`, and CI runs the unittest suite, not `validate`), and plain text matches the shape checkers of this kind already use, printing `ok    ...` or `FAIL  ...` lines. Do not "fix" this back to JSON; there is no reader for it.
