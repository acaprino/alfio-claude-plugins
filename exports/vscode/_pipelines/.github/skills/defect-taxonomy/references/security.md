...
- **Detection**: Search for `* 0.01`, `price: float`, `amount: Number`; check model schemas for `float`/`double` columns on money fields; scan for `round(...)` on totals (hides underlying drift).
