...
- **Detection**: Config audit, search for hardcoded URLs/IPs, locale-sensitive test failures
...
- **Detection**: Search for mock/patch of heavy deps (ortools, scipy, prometheus_client, ML libs) in sub-directory conftest files; verify root `tests/conftest.py` has them instead
