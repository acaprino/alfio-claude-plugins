...
The script is POSIX shell. On Windows it needs Git Bash or WSL, and it auto-detects the platform and switches to foreground mode, which blocks the `#execute/runInTerminal` call. Launch it in a dedicated terminal so the server survives across turns, then read `$STATE_DIR/server-info` on the next turn to get the URL and port.
