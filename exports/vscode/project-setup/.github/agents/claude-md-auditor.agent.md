...
<!-- Vendored from plugins/project-setup/agents/claude-md-auditor.md in acaprino/claude-code-daodan, MIT. -->
...
`testing` plugin in acaprino/claude-code-daodan. The condensed block carries only the
universally binding, review-verifiable rules; procedure (layer budgets, quarantine
mechanics, TEST_AUDIT.md format, consolidation workflow) stays in that plugin.
-->
...
Why the block is short: it carries only rules that are universally binding and verifiable at review time. Everything procedural (layer budgets, quarantine mechanics, audit format, consolidation workflow) lives in the `testing` bundle of this catalog: its test-hygiene knowledge base and the `/test-audit` and `/test-consolidate` prompts. That is a pointer for humans reading this agent definition; the canonical block above stays self-contained on purpose (target projects need no bundle installed for the rules to bind).
...
- **File paths** - all referenced paths exist via `#search/fileSearch`
...
When surfacing a dedup candidate via `#vscode/askQuestions`, the question MUST include:
...
- **Every applied change traces back to an explicit per-drift user decision in this session.** No autonomous batch-fix, even for Critical findings. The agent surfaced each drift via `#vscode/askQuestions` and acted only on the user's chosen option.
...
- All file paths verified with `#search/fileSearch`
