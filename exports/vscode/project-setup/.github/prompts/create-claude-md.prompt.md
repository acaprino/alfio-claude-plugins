...
> **Which file this writes.** Upstream this targets `CLAUDE.md`, Claude Code's project-instruction
> file. On Copilot the equivalent is `AGENTS.md` or `.github/copilot-instructions.md`, and everything
> below applies to those unchanged: same structure, same ground-truth verification, same size
> discipline. Ask the user which filename they want before writing, and default to `AGENTS.md` when a
> project has no instruction file yet. If the project already has a `CLAUDE.md`, work on that.
...
Before the agent starts its own bottom-up analysis, check whether the project already has technical-reference output on disk from a previous `/xray-team-analyze` in the `_pipelines` bundle session:
...
```
Found .deep-dive/ from a previous `/xray-team-analyze` in the `_pipelines` bundle session.
...
- `/xray-team-analyze` in the `_pipelines` bundle - Run first to generate `.deep-dive/` technical references; this command can then ingest them as the structure backbone
