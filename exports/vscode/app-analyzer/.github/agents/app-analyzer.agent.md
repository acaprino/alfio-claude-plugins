...
name: app-analyzer
description: >
  Comprehensive app analysis agent for Android (ADB) and web (Playwright MCP) applications.
  Auto-detects platform. Phase 1 maps the full navigation structure with exhaustive BFS
  exploration. Phase 2 analyzes design system, UX patterns, psychology, business model, and
  generates competitive intelligence reports. Use when analyzing competitor apps, mapping app
  navigation, extracting design systems, or conducting UX audits.
user-invocable: true
agents: []
---

<!-- No `tools:` allowlist: this agent drives a browser through the playwright-mcp MCP
     server, whose tool ids depend on the name the user gives that server, so they cannot
     be allowlisted here. Omitting the field grants the full available tool set. -->

<!-- Vendored from plugins/app-analyzer/agents/app-analyzer.md in acaprino/claude-code-daodan, MIT. -->
...
Web mode requires the [playwright-mcp](https://github.com/microsoft/playwright-mcp) MCP server for its browser tools (`browser_navigate`, `browser_snapshot`, `browser_click`, and the rest). If those tools are not available, STOP and tell the user:

```
Playwright MCP tools are not available.

Web app analysis requires them. Add the playwright-mcp server under
Settings > AI > Manage MCP Servers, then re-run.
...
# B) Analyze screenshot visually (`#read/readFile`)
