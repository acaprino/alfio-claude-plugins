---
description: Analyze and remediate technical debt - inventory debt items, score by impact, and produce a prioritized remediation plan with estimated effort. Covers code debt, architecture debt, testing debt, documentation debt, and infrastructure debt with ROI calculations and quarterly roadmap. Use when the user asks for a technical-debt inventory, debt-score dashboard, prioritized remediation roadmap with effort/ROI estimates, debt-prevention quality gates, or stakeholder-facing debt report. Multi-language and multi-dimension (code, architecture, testing, docs, infra). Not for a Python-only lint, type and complexity audit, which `/python-audit` in the `python-development` bundle covers, dead-code and dependency removal alone, which `/team-review` in the `_pipelines` bundle covers, or a single-file refactoring pass, which `/python-refactor` in the `python-development` bundle and `/clean-code` in the `clean-code` bundle cover.
