...
2. Ask the script whether a serper key is available: run `python $SKILLS/web-search-techniques/scripts/websearch.py --check-key` in the terminal. Exit 0 means yes (it names the source, the environment or the key file), exit 2 means no. The check makes no network call, so it costs no credit.
...
4. Collecting a key in chat (only in the case named above). Ask with `#vscode/askQuestions`, one question, these options:
   ...
   With a key in hand, save it through the script rather than by writing the file yourself, so the permissions and the format are the script's business. Run this in the terminal:

   ```bash
   printf '%s' '<the key the user pasted>' | python $SKILLS/web-search-techniques/scripts/websearch.py --set-key
   ```
