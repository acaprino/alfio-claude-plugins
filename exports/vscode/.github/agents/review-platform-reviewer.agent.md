---
name: review-platform-reviewer
description: >
  Adversarial cross-platform code reviewer. Audits code against the platform-engineering rulebook: server
  validation, auth token storage, API security, XSS and CSP, secrets exposure, architecture patterns, and
  performance, across SPA, PWA, mobile, and desktop (Electron, Tauri). Runs as the Platform compliance
  dimension of /team-review on fullstack projects.
...
<!-- Vendored from plugins/platform-engineering/agents/platform-reviewer.md in acaprino/claude-code-daodan, MIT. -->
