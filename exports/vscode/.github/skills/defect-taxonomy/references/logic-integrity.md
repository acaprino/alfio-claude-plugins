...
- **Detection**: Cross-reference `.team-review/02-interconnect.md` `## Contracts -> Implicit` rows marked `unverified`; search for every caller of the function; verify each path enforces P.
...
Unlike other taxonomy categories, logic-integrity defects cannot be found reliably through static analysis tools. They require **semantic context** (the `.team-review/02-interconnect.md` map) to identify. Detection workflow for `review-logic-integrity-auditor`:
