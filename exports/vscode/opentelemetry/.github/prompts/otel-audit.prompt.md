...
- Async context propagation patterns -> the `async-python-patterns` skill in the `python-development` bundle
- RabbitMQ propagation -> the `rabbitmq-expert` agent in the `messaging` bundle, plus this bundle's `opentelemetry` skill
