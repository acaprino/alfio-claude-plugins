...
- **Orders staged with `transmit=False` are invisible to `reqOpenOrders`/`reqAllOpenOrders`** (observed, maintainer-confirmed): a reconnecting client cannot rediscover staged bracket legs from the venue. Keep your own record of staged ids, and call `reqOpenTrades()` after transmitting a leg mid-session so the library resyncs.
