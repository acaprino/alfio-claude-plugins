...
- **Live URL mode:** if the argument starts with `http://` or `https://`, drive the page with playwright-mcp, parse the manifest, test install criteria, check security headers, test offline behavior, and measure Core Web Vitals on the landing page.
...
Read the relevant references from the `pwa-development` skill on demand rather than all upfront, targeting each section of the audit.
...
If the target is a URL, drive the browser with the [playwright-mcp](https://github.com/microsoft/playwright-mcp) MCP server's tools:
...
If no browser-automation tools are available, tell the user to add playwright-mcp under **Settings > AI > Manage MCP Servers**, and meanwhile fall back to suggesting manual checks, explaining what each would verify. In that fallback path the audit must still produce a structured report, with explicit "could not verify" markers on every item that required browser automation.

Live-mode auth note: if the target URL is behind authentication, ask with `#vscode/askQuestions` whether to audit a public route only, or whether the user can supply login steps. Never attempt to bypass auth.

## Mode B: Local code

If `$ARGUMENTS` is a path or omitted, search the codebase:

1. Find the manifest: `#search/fileSearch` for `manifest.webmanifest`, `manifest.json`, `app/manifest.ts` (Next.js 14+), `src/manifest.ts`. Parse and verify against `production-checklist.md` §1.
2. Find the SW source: `#search/fileSearch` for `sw.js`, `sw.ts`, `service-worker.js`, `service-worker.ts`, `app/sw.ts` (Serwist), `ngsw-config.json` (Angular). Read and audit caching strategies, `skipWaiting` / `clients.claim` use, cleanup in `activate`, offline fallback.
3. Find the SW registration: `#search/textSearch` for `serviceWorker.register`, `workbox-window`, `Workbox(`, `Serwist(`, or Next.js implicit registration via `@serwist/next`.
4. Find iOS meta tags: `#search/textSearch` for `apple-mobile-web-app-capable`. Confirm the full block from `manifest.md` §"iOS meta tag block".
...
6. Find Web Push: `#search/textSearch` for `pushManager.subscribe`, `setVapidDetails`, `web-push`. If found, audit the subscription flow and server-side delivery against `push-notifications.md`.
7. Find storage usage: `#search/textSearch` for `localStorage.`, `IDBDatabase`, `openDB(`, `navigator.storage`. Flag anti-patterns from `storage-persistence.md` (for example, `localStorage` used for large data).
