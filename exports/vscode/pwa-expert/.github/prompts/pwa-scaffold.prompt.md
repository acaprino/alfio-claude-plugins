...
Scaffold a production-ready PWA into the current project. Read the relevant references (`manifest.md`, `service-workers.md`, `frameworks-tooling.md`) on demand.
...
4. If detection is ambiguous, ask with `#vscode/askQuestions` which framework to target.

## Inputs to collect

Before generating files, use `#vscode/askQuestions` to gather:
...
Before writing each file, check if it exists. If it does, show a diff and ask with `#vscode/askQuestions` whether to overwrite, merge, or skip. Never silently overwrite an existing manifest or SW.
...
- Run `ng add @angular/pwa --project <project-name>` with `#execute/runInTerminal`. This schematic creates the manifest, the SW, registration, and `ngsw-config.json`.
...
- A short "Next steps" message in the chat: open the manifest in DevTools Application panel, replace the icon stubs with real icons, generate `apple-touch-startup-image` for the target devices (suggest `pwa-asset-generator`), run `/pwa-checklist` once the app is wired up.
