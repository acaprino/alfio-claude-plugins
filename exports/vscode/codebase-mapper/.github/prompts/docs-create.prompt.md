---
description: Documents a project from its source, on one dimension or many. Use when the user asks to create technical documentation, API docs, architecture guides, data model / schema docs, data flow / pipeline docs, dependency maps, or any new documentation for a codebase. Not for auditing existing docs (use /docs-maintain), a README alone (use /maintain-readme in the `docs` bundle), or a full multi-document project guide (use /map-codebase).
