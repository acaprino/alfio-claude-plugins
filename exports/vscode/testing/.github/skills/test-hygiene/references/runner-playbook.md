<!-- Vendored from plugins/testing/skills/test-hygiene/references/runner-playbook.md in acaprino/claude-code-daodan, MIT. -->

# Runner Playbook

Detection signals and measurement commands per test runner. Used by `/test-audit`, `/test-consolidate`, and the test-suite-auditor agent. Every command here is read-only with respect to the code; commands that RUN the suite are marked, and the caller decides whether running is permitted (`--no-run`).
...
|---|---|---|---|---|
| pytest | `pytest --collect-only -q` | `pytest -q --durations=20` | search `skip\|xfail` markers; `pytest -rs` summary | `pytest --cov=<pkg> --cov-report=term-missing` (pytest-cov) |
| Vitest | `vitest list` | `vitest run --reporter=verbose` | search `\.skip\|\.todo`; reporter summary | `vitest run --coverage` |
| Jest | `jest --listTests` | `jest --verbose` | search `\.skip\|xit\|xdescribe`; reporter summary | `jest --coverage` |
| Mocha | `mocha --dry-run` (10+) | `mocha --reporter spec` | search `\.skip`; pending count in output | via nyc: `nyc mocha` |
| go test | `go test -list '.*' ./...` | `go test -v ./...` (per-test timing in output) | search `t\.Skip`; `--- SKIP` lines | `go test -coverprofile=cover.out ./...` then `go tool cover -func=cover.out` |
| cargo test | `cargo test -- --list` | `cargo test -- --nocapture` (use `cargo nextest run` for per-test timing when available) | search `#\[ignore\]`; `ignored` count in output | `cargo llvm-cov` or `cargo tarpaulin` when configured |
| Gradle | `gradle test --dry-run` (approx) | `gradle test` then read `build/test-results/test/*.xml` (has per-test `time`) | `@Disabled\|@Ignore` search; `skipped` attr in XML | JaCoCo report when the plugin is applied |
...
| dotnet | `dotnet test --list-tests` | `dotnet test --logger "trx"` (per-test times in trx) | `Skip=` search; `skipped` in summary | `dotnet test --collect:"XPlat Code Coverage"` |
| RSpec | `rspec --dry-run -f doc` | `rspec --profile 20` | search `skip\|pending\|xit`; pending count | SimpleCov when configured |
| PHPUnit | `phpunit --list-tests` | `phpunit --log-junit report.xml` (per-test times) | `markTestSkipped` search; summary | `phpunit --coverage-text` (xdebug/pcov) |
