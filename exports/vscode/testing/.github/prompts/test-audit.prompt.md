...
<!-- Adapted from plugins/testing/commands/test-audit.md in acaprino/claude-code-daodan, MIT.
     The upstream Step 3 dispatches the test-suite-auditor agent; here the qualitative audit
     runs inline (following that agent's detection pipeline) because binding the prompt to
     that agent would put the whole run, including the --fix quarantine moves, under its
     report-only contract. -->

# /test-audit

Whole-suite health audit. Produces a versioned `TEST_AUDIT.md` at the target root; with `--fix`, moves the confirmed rot (orphan, long-skipped, failing, flaky tests) into `tests/_quarantine/` in gated, individually revertible commits. Measurement and quarantine only: consolidation is `/test-consolidate`.
...
1. Consult the `test-hygiene` skill. Its `references/runner-playbook.md` drives Steps 1 and 2; its `references/remediation-workflow.md` defines the `TEST_AUDIT.md` format and the quarantine protocol used below.
...
Collect via the terminal (`execute/runInTerminal`), using the playbook's per-runner commands:

1. Test file list and per-layer counts (list-tests command; no execution).
2. Skip/disable marker counts (marker table from the skill's `prevention-rules.md` section 6).
...
Run the judgment dimensions inline, following the detection pipeline of this bundle's `test-suite-auditor` agent definition: D2 orphans, D4 failing/flaky classification, D5 duplicates, D6 contradictions, D7 implementation-coupled, D8 never-failing. Apply its prime directives (evidence or nothing; search before flagging; false-positive candidates in their own section; every finding names its fix path) and its severity scale. Write the full findings report to `<path>/.test-audit/findings.md`. Nothing moves in this step.
...
Gated shape: acceptance gate, baseline, one commit per batch, verification gate between batches, hard restore on failure.
...
Present the non-empty categories via `#vscode/askQuestions` as a multi-select (label: category and file count; description: sample paths). Only accepted categories proceed. This gate is never bypassed: `--yes` affects only the per-batch re-confirmation below.
...
- `/test-consolidate <module>` for the modules ranked worst by duplicate/implementation-coupling density in Step 3.
...
- `<path>/.test-audit/findings.md`: the full findings report backing the latest audit section.
