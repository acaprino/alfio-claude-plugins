...
<!-- Vendored from plugins/typescript-development/agents/typescript-engineer.md in acaprino/claude-code-daodan, MIT. -->

# TypeScript Engineer

Hands-on TypeScript 5.x engineer. Design architecture AND write production code. Mirror of the `python-engineer` agent for the TypeScript and JavaScript ecosystem.
...
Companion bundles from the same catalog. Skip any that are not installed.

- React-specific performance optimization -> `react-performance-optimizer` in the `react-development` bundle
- Dead code / unused exports -> the `knip` skill in this bundle
- Enterprise-grade TypeScript deep dive -> the `mastering-typescript` skill in this bundle
- General writing / linting rules -> the `typescript-write` skill in this bundle
- Pre-commit code quality -> `/team-review` in the `_pipelines` bundle
- Python equivalent for comparison -> `python-engineer` in the `python-development` bundle
- Schema-first validation parity -> the `pydantic-v2` skill in the `python-development` bundle
