...
<!-- Vendored from plugins/libgdx-development/agents/libgdx-architect.md in acaprino/claude-code-daodan, MIT. -->
...
- **the `tdd` skill in the `testing` bundle**: GameTest/HeadlessApplication for unit-testing render-free logic; integration tests for systems
- **multi-stage-dockerfile** (in the `docker` bundle): for headless server-side gameplay simulation or build CI containers
