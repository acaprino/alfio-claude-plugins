...
description: >
  ...
  lateral thinking workflow. Uses 4 lateral thinking techniques (semantic collision, vocabulary
  shift, invisible hinge, polarization) for creative generation, then filters with 7 naming
  archetypes, linguistic/phonotactic rules, weighted scoring, domain availability checks, market
  saturation analysis (existing apps, websites, businesses with same name), trademark
  pre-screening, and SEO analysis. Use when "brand name", "naming", "name my app", "name my
  product", "product name", "startup name", "come up with a name", "nome del brand", "naming
  strategico".
user-invocable: true
license: MIT
metadata:
...
```bash
python "$SKILLS/domain-hunter/scripts/domain_checker.py" name1 name2 name3
```

`$SKILLS` is the first of `.github/skills/`, `.agents/skills/`, `.claude/skills/`, `~/.copilot/skills/` that exists. The script checks `.com`, `.app`, `.io` and `.co` availability via a WHOIS API; read its header for setup instructions.

If no API key is available, fall back to web searches such as `"namexyz.com" site:whois` (this needs a web-search tool, see the bundle README) or check registrar sites manually.
