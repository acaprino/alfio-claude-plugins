...
<!-- Vendored from plugins/digital-marketing/agents/seo-specialist.md in acaprino/claude-code-daodan, MIT. -->
...
Fallback: WebFetch for simple HTTP checks, curl via `#execute/runInTerminal` for headers and status codes.
...
- **JS SEO check**: compare `#web/fetch` output (static HTML) vs `browser_snapshot` (rendered DOM) -- flag any critical text, headings, links, or structured data present only in the rendered version, as search engines may not index JS-rendered content reliably
