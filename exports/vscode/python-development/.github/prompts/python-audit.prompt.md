...
- Deep refactoring with metrics -> `/python-refactor`
- Dead code removal only -> the `python-dead-code` skill, or the `_pipelines` bundle's `/team-review` for a codebase-wide hygiene pass
- Adding tests to raise coverage -> `python-tdd` skill
- Agent-instruction file updates after cleanup (`AGENTS.md`, `.github/copilot-instructions.md`) -> the `project-setup` bundle of the same catalog
