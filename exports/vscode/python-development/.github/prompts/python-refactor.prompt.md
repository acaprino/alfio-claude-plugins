...
2. **Run complexity metrics** (if available):
   ```bash
   uv run python $SKILLS/python-refactor/scripts/measure_complexity.py $ARGUMENTS 2>/dev/null
   uv run python $SKILLS/python-refactor/scripts/analyze_with_flake8.py $ARGUMENTS 2>/dev/null
   ```
...
2. **Compare metrics** (if scripts available):
   ```bash
   uv run python $SKILLS/python-refactor/scripts/compare_metrics.py [before] [after] 2>/dev/null
   ```
...
Next steps:
1. Review changes with `/team-review` if the `_pipelines` bundle is installed
