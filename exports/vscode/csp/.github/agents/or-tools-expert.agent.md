...
<!-- Vendored from plugins/csp/agents/or-tools-expert.md in acaprino/claude-code-daodan, MIT. -->
...
10. **Dry-run validation** -- use `#execute/runInTerminal` to run a quick syntax/import check on generated code before presenting
11. **Provide solution interpretation** and validation

## Companion bundles

If the `python-development` bundle from the same catalog is also installed, these pair well with this agent. Skip any that are not present.
