...
Scripts are addressed relative to the installed skills directory. Probe these in order and
use the first that exists, calling it `$SKILLS`: `.github/skills/`, `.agents/skills/`,
`.claude/skills/`, `~/.copilot/skills/`.

All scripts then live at `$SKILLS/stripe/scripts/<name>.py`. Run them with `#execute/runInTerminal`.
...
- Pricing, tier design, revenue projections -> `revenue-optimizer` agent (same bundle)
- API implementation, webhooks, Connect, subscriptions -> `stripe-integrator` agent (same bundle)
- Webhook audit (Stripe account + codebase) -> `stripe-webhooks-auditor` agent + `/audit-webhooks` command
- GDPR / PCI posture around payment data -> `privacy-doc-generator` in the `business` bundle
- Cross-platform token/auth handling in a payment flow -> `platform-reviewer` in the `platform-engineering` bundle
