...
9. **Never end a turn waiting.** Wherever the run needs the operator, it asks with
   `#vscode/askQuestions`; wherever it stops for good, it says the run is over and why.
   A turn that ends on a printed question is indistinguishable from a finished run, and
...
| `sent/r2.md`, `sent/r3.md`, `sent/certification.md`, `sent/corrective.md` | the orchestrator, one file per outgoing payload after round 1 | 4, 5 |

**Every outgoing payload is a file before it is a request.** The transport reads the
payload off disk and takes no inline text (`content_path`, see `server.py`), so each
...
4. If this happened on the very first call (Phase 2, round 1), there is no ledger yet
   ...
   `#vscode/askQuestions`** whether to send the same packet again or end the run. A
   timeout is the common case here and it is transient: the challenger is thinking and
   the socket cannot tell that apart from a dead connection. Retrying resends
   ...
   prompt by hand, and do not attribute the failure to the gateway without evidence:
...
0. **Digest the packet file.** Phase 1 hashed the *artifact* and its embedding. What
   travels is the whole packet, so hash the packet file itself with `#execute/runInTerminal`
   and carry the value as `<PACKET_SHA>` for the rest of the run:
...
3. **Ask, using `#vscode/askQuestions`.** Do not end the turn on printed text: to the
...
- **`--dry-run`**: stop here, before the question. Report the packet path
  (`<run directory>/00-packet.md`) and that nothing was sent. End the run.
- **Consent is read as intent, never as a token.** The operator answers a question;
  they are not entering a password. `Send it`, `ok`, `yes`, `si`, `vai`, `procedi`,
...
2. Call the `peer_ask` tool: `profile` is the resolved profile name, `system` is the
   Round 1 prompt text, `content_path` is `<run directory>/00-packet.md`. Pass the
   path. The packet is never pasted into the call: the transport reads it, and that is
   the only reason R15's third link holds.
...
4. **Verify what was sent (R15).** Compare the reply's `sent_sha256` against
   ...
   section.
5. On success, write the `text` field verbatim to `01-challenge-r1.md` with
...
3. **Write the payload, then send it.** Write step 2's payload to `sent/r<r>.md` in the
   run directory with `#edit/createFile`, then call the `peer_ask` tool with this
   prompt as `system` and `content_path` pointing at that file. The file is not a copy
   of what is sent, it is what is sent: the transport accepts no inline text. Handle
   the error shape per "Transport error handling" above.
4. On success, write the reply to the round's challenge file with `#edit/createFile`.
   ...
   and the reply's `sent_bytes` and `sent_sha256` for the verdict's transmission
   section.
...
2. Load the Certification prompt: read
   ...
   Write the collected findings plus renderings to `sent/certification.md`, then call
   the `peer_ask` tool with the prompt as `system` and `content_path` pointing at that
   file. Handle the error shape per "Transport error handling" above.
...
4. **Corrective round, at most once for the whole run**, only for findings reverted in
   ...
   - Write only the reverted findings' original verbatim claims and falsifiers to
     `sent/corrective.md`, then call the `peer_ask` tool once more with `content_path`
     pointing at that file and `system` built from the same Challenge round prompt
     semantics but scoped to "answer against your original claim, this is a corrective
     round, not a debate" framing. Handle the error shape: on
...
11. **Transmission and token accounting**, from every `peer_ask` call's `usage`,
    ...
    call: what the transport says it sent, not what the orchestrator intended to send.
