...
3. **Validate the artifact.** Read the file at the given path with `#read/readFile`.
   ...
   - This prompt reviews plans and specs, not code changes.
...
5. **Resolve the challenger profile.** Call the `peer_profiles` tool with no arguments.
   ...
   - If `default` is null and no `--challenger` was given: stop. No profile is
     configured. Point at `$SKILLS/cross-model-peer-review/mcp/profiles.example.json`
     and the `.peer-review/profiles.json` / `~/.peer-review/profiles.json` locations it
     can be placed at.
   - If the chosen name does not appear in `profiles`: stop, listing the configured
     profile names, pointing at
