...
Read `.codebase-map/_internal/context-brief.md` first, especially the `## Project Profile` and `## Why / Context` sections. Also read `${PLUGIN_ROOT}/skills/codebase-mapper/references/audience-adaptation.md`. Use the codebase itself to verify and expand on the brief.
