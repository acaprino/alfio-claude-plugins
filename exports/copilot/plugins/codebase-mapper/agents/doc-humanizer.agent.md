...
Humanize toward the target register (from the caller or inferred via `${PLUGIN_ROOT}/skills/codebase-mapper/references/audience-adaptation.md`), not a single fixed tone. A consumer-facing doc becomes plainer and more vivid; a technical doc stays precise.
