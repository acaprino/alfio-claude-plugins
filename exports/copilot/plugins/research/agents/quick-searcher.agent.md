...
2. Pick the most direct path: search for discovery, `WebFetch` for extraction; with `SERPER_API_KEY` set, `python ${PLUGIN_ROOT}/scripts/websearch.py "<query>"` is an equivalent discovery step
...
- **Bash**: only for `${PLUGIN_ROOT}/scripts/websearch.py` (serper backend) and `${PLUGIN_ROOT}/scripts/webfetch.py` (bot-block fallback).
