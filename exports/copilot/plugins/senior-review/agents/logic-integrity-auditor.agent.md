...
- **Always load:** `${PLUGIN_ROOT}/skills/defect-taxonomy/references/logic-integrity.md` -- the 8 categories (L1-L8) with CWE mappings, detection strategies, fix patterns, signatures
- **Load on demand:** `${PLUGIN_ROOT}/skills/defect-taxonomy/references/review-frameworks.md` -- scoring, anti-pattern checklist (only if producing the Code Quality Score section)
- **Load on demand:** `${PLUGIN_ROOT}/skills/defect-taxonomy/references/concurrency-state.md` -- when interconnect map flags concurrency contracts (L5.2 reentrancy, L6 idempotency under concurrency)
