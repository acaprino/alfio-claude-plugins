...
> Arguments: `[question, code, or contract]`. Wherever `<arguments>` appears below, substitute the text the user typed after the prompt name.
...
```bash
python ${PLUGIN_ROOT}/skills/ibkr/scripts/ibkr_probe.py codes <code> [<code> ...]
```
...
```bash
python ${PLUGIN_ROOT}/skills/ibkr/scripts/ibkr_probe.py capabilities --stock AAPL
```
