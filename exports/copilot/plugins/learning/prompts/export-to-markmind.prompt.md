...
> Arguments: `<topic | \"text to map\" | path/to/file.md>`. Wherever `<arguments>` appears below, substitute the text the user typed after the prompt name.
...
2. **Render to MarkMind** (skill: `learning:markmind-exporter`): pipe the JSON outline to `${PLUGIN_ROOT}/skills/markmind-exporter/scripts/generate_markmind.py` with `--output` flag to produce the `.md` file.
