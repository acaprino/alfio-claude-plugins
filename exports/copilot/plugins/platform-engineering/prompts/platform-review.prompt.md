...
> Arguments: `[target-path] [--platform spa|pwa|mobile|electron|tauri|auto] [--focus security|arch|perf]`. Wherever `<arguments>` appears below, substitute the text the user typed after the prompt name.
