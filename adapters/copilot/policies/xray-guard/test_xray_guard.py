...
CASES = [
    # --- X-ray workers: confined to .codebase-xray ---
    ("worker writes owned phase file",
     pre("edit/createFile", {"filePath": ".codebase-xray/runs/r1/partitions/api/01-structure.md"}),
     ".codebase-xray", "allow"),
    ("worker creates run subdir",
     pre("edit/createDirectory", {"dirPath": ".codebase-xray/runs/r1/partitions/web"}),
     ".codebase-xray", "allow"),
    ("worker writes source file",
     pre("edit/createFile", {"filePath": "src/main.py"}), ".codebase-xray", "deny"),
    ("worker edits source file",
     pre("edit/editFiles", {"filePath": "src/auth/login.ts", "explanation": "fix"}),
     ".codebase-xray", "deny"),
    ("worker reads source file",
     pre("read/readFile", {"filePath": "src/main.py"}), ".codebase-xray", "allow"),

    # --- secret protection ---
    ("reads .env", pre("read/readFile", {"filePath": "src/.env"}), ".codebase-xray", "deny"),
    ("reads .env.production",
     pre("read/readFile", {"filePath": ".env.production"}), ".codebase-xray", "deny"),
    ("reads a private key",
     pre("read/readFile", {"filePath": "deploy/certs/server.pem"}), ".codebase-xray", "deny"),
    ("reads id_rsa",
     pre("read/readFile", {"filePath": "infra/keys/id_rsa"}), ".codebase-xray", "deny"),
    ("reads .netrc", pre("read/readFile", {"filePath": ".netrc"}), ".codebase-xray", "deny"),
    ("reads a file inside a secrets dir",
     pre("read/readFile", {"filePath": "config/secrets/db.yaml"}), ".codebase-xray", "deny"),
    ("reads secrets.yaml",
     pre("read/readFile", {"filePath": "config/secrets.yaml"}), ".codebase-xray", "deny"),
    ("reads a source module named secrets_manager.py",
     pre("read/readFile", {"filePath": "src/auth/secrets_manager.py"}), ".codebase-xray", "allow"),
    ("reads a source module named credential_store.ts",
     pre("read/readFile", {"filePath": "src/credential_store.ts"}), ".codebase-xray", "allow"),
    ("reads keyboard.py, not a *.key match",
     pre("read/readFile", {"filePath": "src/ui/keyboard.py"}), ".codebase-xray", "allow"),
    ...
    ("terminal runs an analysis script on source",
     ...
     ".codebase-xray", "allow"),
    ("terminal cats .env",
     pre("execute/runInTerminal", {"command": "cat ./.env"}), ".codebase-xray", "deny"),

    # --- path edge cases ---
    ("path traversal out of the confine",
     pre("edit/createFile", {"filePath": ".codebase-xray/../src/evil.py"}), ".codebase-xray", "deny"),
    ("absolute path inside the confine",
     pre("edit/createFile",
         {"filePath": os.path.join(WORKSPACE, ".codebase-xray", "runs", "r1", "state.json")}),
     ".codebase-xray", "allow"),
    ...
    ("multi-file payload with one bad target",
     pre("edit/editFiles",
         {"filePaths": [".codebase-xray/runs/r1/07-final-report.md", "README.md"]}),
     ".codebase-xray", "deny"),
    ("free-text field naming a source path is not a target",
     pre("edit/createFile",
         {"filePath": ".codebase-xray/runs/r1/05-risks.md",
          "explanation": "documents src/auth/login.py and .env handling"}),
     ".codebase-xray", "allow"),
    ("non-mutating search tool outside the confine",
     pre("search/textSearch", {"query": "TODO", "path": "src/"}), ".codebase-xray", "allow"),
    ...
    ("reviewer writes its findings file",
     ...
     ".team-review", "allow"),
    ("reviewer writes into .codebase-xray, not its session",
     pre("edit/createFile", {"filePath": ".codebase-xray/runs/r1/01-structure.md"}),
     ".team-review", "deny"),
    ...
    ("reviewer reads the X-ray run output",
     pre("read/readFile", {"filePath": ".codebase-xray/runs/r1/05-risks.md"}),
     ".team-review", "allow"),
    ...
    # --- fail-open behavior ---
    ("malformed json fails open", "{not json", ".codebase-xray", "allow"),
    ("empty payload fails open", "{}", ".codebase-xray", "allow"),
    ("non-PreToolUse event passes through",
     {"hook_event_name": "PostToolUse", "tool_name": "edit/createFile",
      "tool_input": {"filePath": "src/main.py"}}, ".codebase-xray", "allow"),
    ("unknown mutating tool name fails open",
     pre("some/futureWriteTool", {"filePath": "src/main.py"}), ".codebase-xray", "allow"),
]
