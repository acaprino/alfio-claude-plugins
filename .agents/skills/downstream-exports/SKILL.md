...
description: >
  How to mirror our content OUT to hosts that are not Codex: the `exports/vscode/`
  ...
  content that deliberately keeps Codex vocabulary, the method for a full re-audit, and
...
The mirror image of the `external-repo-intake` skill. That skill covers content flowing *in* from other repos; this one covers our content flowing *out* to hosts that are not Codex. `exports/<host>/` holds those ports.
...
```bash
python .Codex/skills/downstream-exports/scripts/gen_extension_manifest.py
```
...
1. **Frontmatter conversion.** Codex `name` / `description` / `model: inherit` / `color` / `tools: Read, Write, ...` becomes VS Code `name` / `description` / `user-invocable` / `tools:` (YAML list of namespaced ids) / `agents:` / `hooks:`. Drop `model` and `color`. Skills add `license: MIT` and a `metadata` block (author, `source: acaprino/Codex-daodan`, `upstream-plugin`). Prompts keep only `description`, optional `agent`, optional unquoted `argument-hint`.
...
6. **`${CLAUDE_PLUGIN_ROOT}`** -> `$SKILLS`, defined once per file as the first of `.github/skills/`, `.agents/skills/`, `.Codex/skills/`, `~/.copilot/skills/` that exists. Any file that uses `$SKILLS` must define it.
...
11. **De-branding.** "Codex" as the actor becomes "the agent" or "the AI". See the exclusions below before running this over a whole bundle.
...
## Content that keeps Codex vocabulary

Two bundles have Codex as their **subject matter**. Their tool names, `TRIGGER WHEN` labels and `CLAUDE_CODE_*` variables are the material being taught, and renaming them makes the content false:

- **`marketplace-ops`** (the whole bundle) authors Codex plugin marketplaces.
- **`ai-tooling/.github/skills/agent-sdk-builder`** documents the Codex Agent SDK.
...
- **`project-setup` targets `AGENTS.md` / `.github/copilot-instructions.md`**, honoring an existing `AGENTS.md`. A note at the top of each file states the mapping; the substance is unchanged.
...
```bash
python .Codex/skills/downstream-exports/scripts/check_export.py
```

Nine passes: frontmatter schema, tool ids, name uniqueness across bundles, prompt `agent:` bindings, `agents:` allowlists, residual Codex coupling, malformed markdown code spans, byte-copy drift against `plugins/`, and `$SKILLS` defined wherever it is used. It exits non-zero on any failure and needs no dependencies.
...
> `` `$SKILLS` is the installed skills directory: the first of `.github/skills/`, `.agents/skills/`, `.Codex/skills/`, `~/.copilot/skills/` that exists. ``
...
- **"Codex" is often legitimate**: attribution comments, `acaprino/Codex-daodan` in metadata, upstream repo names such as `dchuk/Codex-tauri-skills`, and Codex named as a long-context LLM in a RAG comparison.
