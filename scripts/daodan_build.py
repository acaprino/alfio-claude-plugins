...
from scripts.daodan.catalogs import assert_cross_host_identity, catalog_document, render_catalog  # noqa: E402
...
def marketplace_version(root: Path) -> str:
    version_file = Path(root) / CATALOG_VERSION_FILE
...
def build_repository(root: Path, hosts: tuple[str, ...], check: bool) -> BuildReport:
    ...
    for host in hosts:
        ...
        rendered = render_catalog(host, plugins, version)
        if check:
