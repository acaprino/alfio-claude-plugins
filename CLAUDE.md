...
No build step, no runtime tests: all content is static markdown. There IS a consistency CI (`.github/workflows/consistency.yml`, runs on push to `master` and on PRs) that mechanically enforces contracts which used to live only in this file. Five checks, all stdlib-only Python runnable from the repo root:
...
2. `python scripts/lint_bundled_paths.py` — the bundled-path linter. A body that says to read `plugins/<name>/skills/x/references/y.md` names a path that exists only in a checkout of this repo: Claude Code installs plugins into its own cache, so for an installed user that read silently fails. Self-references must use `${CLAUDE_PLUGIN_ROOT}/...` (or a skill-relative `references/...` path inside that same skill), and no plugin may reach into another plugin's files by path at all. It carries a `GRANDFATHERED` baseline of 33 pre-existing broken references across 22 files in `business`, `codebase-mapper`, `senior-review` and `stripe`: those are real defects awaiting their own pass, keyed by file AND exact path so a new break in a listed file still fails. Fix a reference, delete its entry; never add one.
3. `python .claude/skills/downstream-exports/scripts/check_export.py` — the export structural checker (see the `downstream-exports` skill).
4. `python .claude/skills/downstream-exports/scripts/gen_extension_manifest.py --check` — fails if `exports/vscode/package.json` contribution lists are stale relative to the bundles on disk.
5. `python scripts/check_version_bumps.py <base-rev> [<head-rev>]` — over the pushed or PR commit range, a change under `plugins/<name>/` must come with a bump of that plugin's `version` and of `metadata.version` (steps 1 and 2 of the marketplace update workflow).
