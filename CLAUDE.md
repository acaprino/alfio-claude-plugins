...
| `frontend` (impeccable cherry-pick, Apache-2.0) | `pbakaus/impeccable` - `skill/reference/` | New files: `plugins/frontend/skills/frontend-css/references/{typography,color-and-contrast,motion-design,heuristics-scoring,cognitive-load,personas,visual-intensity,delight-and-microinteractions,production-hardening}.md`, `plugins/frontend/skills/frontend-strategy/references/brand-register.md`. Merged sections (appended, delimited by attribution comment): `plugins/frontend/skills/frontend-css/references/{layout-patterns,ui-pattern-guide,css-patterns,ux-patterns}.md`. 2026-06-02 refresh (snapshot bumped from 2026-05-11): re-diffed typography/color-and-contrast/layout (local already ahead, only layout-patterns gained rhythm + flex-1D/grid-2D + hero-metric nuggets), and cherry-picked 3 new files from upstream `bolder.md`+`quieter.md` (-> visual-intensity), `delight.md` (-> delight-and-microinteractions), `harden.md` (-> production-hardening). `distill.md` intentionally skipped (covered by cognitive-load + ux-patterns). Workflow/harness files (live, codex, critique, craft, overdrive) intentionally not vendored. 2026-06-10 refresh: `brand-register.md` re-diffed (snapshot bumped from 2026-05-11) and updated additions-only: inverse slop test, imagery comp-protection, Unsplash URL verification, uppercase-kicker ban, image-led hero bullet, first-load motion rewording. Upstream deletions/rewrites (category recipes, tech-zero-imagery exception, strict-grid and don't-center bullets, grid-template-rows tip, typographic-risk bullet) intentionally NOT pulled. 2026-07-12 refresh: `visual-intensity.md` gained the Design-System Lock paragraph from upstream's bolder.md rewrite (upstream's deletion of the tactical numbers NOT mirrored; local keeps them; snapshot bumped to 2026-07-12), and 2 new files cherry-picked from the upstream platform axis: `android.md` -> `references/android-platform.md`, `ios.md` -> `references/ios-platform.md` (detector `<!-- rule:* -->` comments stripped). Platform-axis harness additions (adapt.native/audit.native commands, typeset/layout "isolated assessments" blocks, animate platform pointer, critique orchestration) intentionally not vendored. |
...
- **`pbakaus/impeccable` (Apache-2.0)**: preserve `NOTICE.md` chain in `plugins/frontend/NOTICE.md`. Upstream DELETED its repo-root `NOTICE.md` on 2026-06-09; the local NOTICE documents our derived-content lineage and must stay regardless (stop looking for the upstream file). Some upstream files become new local files; others are appended as delimited sections inside existing files. The sync table row spells out the exact split.
