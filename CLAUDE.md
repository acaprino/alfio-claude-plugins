...
4. **Commit together** - stage the kernel files and everything the build regenerated under `exports/` and the three root manifests, in one commit
...
4. Build all three hosts, bump `metadata.version`, and commit the kernel plus the generated output together
...
The build is `python scripts/daodan_build.py`. It loads every kernel, validates it, binds it to each of the three adapters, renders every package into a temporary tree, and replaces the live output only after every validator passes. `--check` renders the same tree and reports drift instead of writing, and it is the gate CI runs. Exit codes: 0 clean, 1 drift or validation failure, 2 invocation error. Publication always builds all three hosts; `--host` narrows the run and is accepted only under `--check`, because a marketplace where one host is a commit ahead of the others is the drift this compiler exists to prevent.
...
`.github/workflows/publish-marketplaces.yml` is the only workflow here that pushes. On every push to `master` it rebuilds all three marketplaces, verifies the built tree reproduces, and commits the result as "Publish native Daodan marketplaces". It publishes all three hosts or none, and the final `--check` runs before the commit rather than after it.
...
Registering the repository root as a marketplace is the whole distribution story, on all three hosts:
...
There is no extension, no `.vsix`, no packaging job and no release tag. The VS Code extension that used to carry the Copilot port was a workaround from before Copilot read plugin marketplaces, and it was removed at the universal cutover along with its release workflow and its skill-copy lifecycle. Historical GitHub Release assets are left untouched and are unsupported. `docs/migration-from-claude-code-daodan.md` holds the one-time migration and the rollback rule: roll back with a revert plus a **new** patch version, never by reusing a published version and never by renaming the repository back.
